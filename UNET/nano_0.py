...
def main():
    ...
    folder_name = "jetson_0" #EDIT CONTAINS JETSON NUMBER
    ...
    LOAD_DIR = "/home/nano/srdsg/Senior_Design_Group_FH7/UNET/jetson_0"  #EDIT CONTAINS JETSON NUMBER
    file_size = os.path.getsize(r"{}/global_model_0.zip".format(LOAD_DIR)) #EDIT CONTAINS JETSON NUMBER
    ...
    with zipfile.ZipFile("{}/{}".format(LOAD_DIR, "global_model_0.zip"), 'r') as zip_ref: #EDIT CONTAINS JETSON NUMBER
        zip_ref.extractall("{}/".format(LOAD_DIR))
        os.remove("{}/{}".format(LOAD_DIR, "global_model_0.zip")) #EDIT CONTAINS JETSON NUMBER
    print("global_model_0.zip has been extracted for training loop and subsequently deleted\n") #EDIT CONTAINS JETSON NUMBER

    print("setting name of trained model file\n")
    trained_model_file_name = "nano_0_trained_model.pth" #EDIT CONTAINS JETSON NUMBER
    ...
    model = TrainingLoop(2, 1, .0001, trained_model_file_name)
    print("training of model complete\n")

    print("zipping the trained model on nano\n")
    with zipfile.ZipFile("trained_model_0.zip", mode = 'w') as archive: #EDIT CONTAINS JETSON NUMBER
    ...
    file_size = os.path.getsize(r"{}/trained_model_0.zip".format(LOAD_DIR)) #EDIT CONTAINS JETSON NUMBER
    ...
    os.remove("{}/{}".format(LOAD_DIR, "trained_model_0.zip")) #EDIT CONTAINS JETSON NUMBER

    print("exiting jetson folder")
    current_dir = os.getcwd()
    if os.path.basename(current_dir) == "jetson_0": #EDIT CONTAINS JETSON NUMBER
        os.chdir('..')
        print(f"Exited {current_dir}, current working directory is now {os.getcwd()}")
    else:
        print(f"Error: current directory is not jetson_0")  #EDIT CONTAINS JETSON NUMBER 
